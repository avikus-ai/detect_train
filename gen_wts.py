...
class Layers(object):
    ...
    # Extra Modules
    # 23.01.16 Stem Block for yolo5-face
    def StemBlock(self, child):
        self.current = child.i
        self.fc.write('\n# StemBlock\n')

        self.convolutional(child.stem_1)
        self.convolutional(child.stem_2a)
        self.convolutional(child.stem_2b)
        self.route('-3')
        self.maxpool(child.stem_2p)
        self.route('-3, -1')
...
with open(wts_file, 'w') as fw, open(cfg_file, 'w') as fc:
    ...
    for child in model.model.children():
        if child._get_name() == 'Focus':
            ...
            layers.Dense(child)
        # Extra Modules
        # 23.01.16 StemBlock for yolo5-face
        elif child._get_name() == 'StemBlock':
            layers.StemBlock(child)
        # 23.01.16 Ghost layers for GhostNet
        elif child._get_name() == 'GhostConv':
            layers.GhostConv(child)
        elif child._get_name() == 'C3Ghost':
            layers.C3Ghost(child)
        else:
