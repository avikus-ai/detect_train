...
file_list = ['2939','ccnuri','GORAE/221020','GORAE/221026','GORAE/221028', 'hannara/busan','hannara/incheon_arriving', 'hannara/incheon_berth','KHOPE', 'maersk/2023-01-15_10/eo1','maersk/2023-01-15_10/eo2','maersk/2023-01-15_10/eo3','maersk/2023-01-15_11/eo1','maersk/2023-01-15_11/eo2','maersk/2023-01-15_11/eo3','YMwinner']

# image, label, sliced result path
for file in file_list:
    img_dataset_folder = sorted(glob.glob("/data/batch01_new/SINGLE_CLASS_revised/" + file + "/images/train/*.jpg"))
    txt_dataset_folder = sorted(glob.glob("/data/batch01_new/SINGLE_CLASS_revised/" + file + "/labels/train/*.txt"))
    sliced_dataset_folder = "/data/batch01_new/SINGLE_CLASS_sliced/"
    for d_idx in range(len(img_dataset_folder)):
        ...
        for p in range(len(lb)):
            ...
            elif min_x >= 0 and min_x < int(w/3) and max_x >= int(w/3) and max_x < int(w/3)*2:# 1st, 2nd frame에 결침
            ...
            elif min_x >= int(w/3) and min_x < int(w/3)*2 and max_x >= int(w/3)*2 and max_x < w:# 2nd, 3rd frame에 결침
        ...
        s1_path = sliced_dataset_folder + file + '/s1/images/'
        s2_path = sliced_dataset_folder + file + '/s2/images/'
        s3_path = sliced_dataset_folder + file + '/s3/images/'
        l1_path = sliced_dataset_folder + file + '/s1/labels/'
        l2_path = sliced_dataset_folder + file + '/s2/labels/'
        l3_path = sliced_dataset_folder + file + '/s3/labels/'
        ...
        if len(l1)>0:
            ...
            cv2.imwrite(s1_path + img_file_name + '.jpg', img)
            save_array_to_file(l1, l1_path + txt_file_name + '.txt')

        elif len(l1)==0:
            back1_path = sliced_dataset_folder + file + '/s1/background/'
        ...
        if len(l2)>0:
            ...
            img = 255*(img) # *255 or IMAGENET denorm 방법
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) # RGB 채널
            cv2.imwrite(s2_path + img_file_name + '.jpg', img)
            save_array_to_file(l2, l2_path + txt_file_name + '.txt')

        elif len(l2)==0:
            back2_path = sliced_dataset_folder + file + '/s2/background/'
        ...
        if len(l3)>0:
            ...
            img = 255*(img) # *255 or IMAGENET denorm 방법
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) # RGB 채널
            cv2.imwrite(s3_path + img_file_name + '.jpg', img)
            save_array_to_file(l3, l3_path + txt_file_name + '.txt')
            
        elif len(l3)==0:
            back3_path = sliced_dataset_folder + file + '/s3/background/'
