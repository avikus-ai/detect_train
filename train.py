...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze, coco_eval, img_type = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
        opt.resume, opt.noval, opt.nosave, opt.workers, opt.freeze, opt.stop_coco_eval, opt.img_type
    ...
    ch = 1 if img_type.lower() == "ir" else 3
    if pretrained:
        with torch_distributed_zero_first(LOCAL_RANK):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, results_sep, maps, _ = validate.run(data_dict,
            ...
            if fi > best_fitness:
                best_fitness = fi
            log_vals = list(mloss) + list(results_sep) + list(results) + lr
            ...
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _, _ = validate.run(
                        ...
                        callbacks=callbacks,
                        compute_loss=compute_loss,
                        coco_eval=coco_eval)  # val best model with plots
                    if is_coco:
